...
- **Fortuna Health** (fortunahealth.com — the only Medicaid-focused company in the YC map): consumer Medicaid navigation — screening, application, and renewal, 24/7, multilingual, phone-first, distributed via **QR codes handed out at the clinic**. Their stats: 100M+ covered by Medicaid/CHIP/Marketplace; **72% of people who lose Medicaid remain eligible** (churn is administrative); 42-minute average call-center waits. → Built into LumaChart as the patient portal's "Keep your coverage" card: renewal-date tracking + plain-language re-enrollment help. Coverage continuity is care continuity — and for the safety-net beachhead, Medicaid navigation belongs *inside* the record, not in a separate app.
