...
const state = {
  ...
  maidFollowupFiled: false,               // MAID 30-day post-Rx follow-up status filed (demo)
  bcmaScanned: false,                     // VA-style closed-loop medication scan verified (demo)
  selfLog: store.get("selfLog", { week:[30,0,45,20,0,60,15], nourished:4 }), // clinician's own private movement/nutrition log
  _idUploaded: false,
...
function vWellness(){
  ...
  ${(() => { const w = state.selfLog.week, total = w.reduce((a,b)=>a+b,0), goal = 150, pct = Math.min(Math.round(total/goal*100),100);
    return `<div class="card" style="margin-bottom:16px">
    <h3>🥗 Your movement &amp; nutrition <span class="chip green">private to you</span></h3>
    <div class="small" style="margin-bottom:8px">A healthier clinician is a better clinician — physically active physicians counsel patients more effectively and are credible role models.${ev("lobelo")} This log is <b>yours alone</b>: on your device, never wired to productivity, scheduling, or employment — the same promise as Canary. Use it to nudge your own behavior toward what you'd prescribe. Nutrition and exercise are among the biggest levers on a healthy life for everyone, clinicians included — five low-risk lifestyle factors added ~12–14 years of life expectancy in cohorts that included health professionals.${ev("lifestyle")}</div>
    <div class="grid g2">
      <div>
        <div class="metric"><div class="v ${total>=goal?'':''}" style="color:${total>=goal?'var(--green)':'var(--amber)'}">${total} <span style="font-size:15px">min/wk</span></div><div class="l">of the ${goal}-min WHO weekly goal · even 15 min/day added ~3 years of life in a 416,000-person cohort${ev("activity")}</div></div>
        <div class="bar" style="margin-top:6px"><i class="${total>=goal?'green':'amber'}" style="width:${pct}%"></i></div>
      </div>
      <div>
        ${sparkline(w.map((m,i)=>({s:m})), Math.max(...w,30))}
        <div class="tiny">Last 7 days · ${state.selfLog.nourished}/7 nourishing-eating days logged</div>
      </div>
    </div>
    <div style="display:flex; gap:8px; margin-top:12px; flex-wrap:wrap">
      <button class="btn primary small" data-self="15">+15 min moved today</button>
      <button class="btn small" data-self="30">+30 min</button>
      <button class="btn ghost small" data-self="nourish">🥗 Logged a nourishing day</button>
    ...
    ${(() => { const f = MAID.followup, filed = state.maidFollowupFiled;
      ...
      <div class="rowlist">
        ${f.milestones.map(m => `<div class="rowitem">
          ...
          <div style="flex:1"><div class="t small">${m.t} <span class="tiny">· ${m.who}</span>${(m.day===30 && filed)?` <span class="chip green">filed ✓</span>`:""}</div><div class="d">${m.d}</div></div>
        </div>`).join("")}
      </div>
      <div style="display:flex; gap:8px; margin-top:10px; flex-wrap:wrap">
        ${filed
          ? `<span class="chip green">✓ 30-day status filed: “not yet ingested / unknown”</span> <button class="btn ghost small" data-maid-fu-reset>Reset</button>`
          : `<button class="btn primary small" data-maid-followup>File 30-day status — “not yet ingested / unknown”</button>`}
      </div>
      <div class="tiny" style="margin-top:8px">${f.note}${aut("maidNM")}</div>`; })()}
    ` : ""}
  </div>`;
}
...
function vCompete(){
  ...
  <div class="card" style="margin:0 0 16px">
    ...
    <div class="small" style="margin-bottom:8px">The VA's VistA/CPRS is the most-studied public EHR, run across the nation's largest integrated system.${aut("vista")} "Best ideas, improved" means adopting what it got right — with the evidence, and the honest lessons, attached. The flip side is instructive too: the VA's multi-billion-dollar Oracle/Cerner replacement stumbled on cost, schedule${aut("vaEhrm")} and patient safety — its "unknown queue" routed orders nowhere and caused 149 patient-harm events${aut("vaOig")} — big-bang rip-and-replace is exactly what LumaChart's incremental, composition-based, usability-first path avoids. Two VA practices worth adopting directly: its <b>joint Federal EHR governance</b> model (a lifelong record across agencies)${aut("fehrm")} and its <b>published, site-by-site phased deployment</b> with readiness gates${aut("vaDeploy")} — the rollout methodology for LumaChart's own pilot-to-scale path.</div>
    <div class="rowlist">
...
function wireView(){
  ...
  // --- clinician's own private movement/nutrition log ---
  $$("[data-self]").forEach(b => b.addEventListener("click", () => {
    const v = b.dataset.self, sl = state.selfLog;
    if (v === "15" || v === "30") { sl.week[sl.week.length-1] += +v; toast("Logged — nice", `+${v} minutes today. Private to you; a quiet mirror, not a scoreboard.`, "green"); }
    else if (v === "nourish") { sl.nourished = Math.min(sl.nourished + 1, 7); toast("Noted 🥗", "A nourishing day logged — for you, not for anyone to review.", "green"); }
  ...
  const maidFu = $("[data-maid-followup]"); if (maidFu) maidFu.addEventListener("click", () => { state.maidFollowupFiled = true; render(); toast("Follow-up filed", "30-day status “not yet ingested / unknown” filed with the state administration. The 6-month deadline is met; the state's 4-month reminder is now moot. (Demo.)", "green"); });
  const maidFuR = $("[data-maid-fu-reset]"); if (maidFuR) maidFuR.addEventListener("click", () => { state.maidFollowupFiled = false; render(); });
